...
export async function findLocalDeals({ providers = [], budget, category, location, limit = 8 } = {}) {
  ...
  const prompt =
    `You are a local-deals researcher with live web access. Find CURRENT, REAL value meals, combos, ` +
    `specials, promotions, or discounts priced at or below $${budgetNum} offered by these ` +
    `${category || 'local'} businesses${location ? ` in ${location}` : ''}. ` +
    `Only include a business where you find a genuine current deal at or under $${budgetNum}; skip the rest. ` +
    ...
    `"title":"<short deal name, e.g. Everyday Value Burrito>","price":"<e.g. $6.99>",` +
